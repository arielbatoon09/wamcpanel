...
export function ServerPlayersTab({ id }: { id: string }) {
  ...
  const PAGE_SIZE = 6;
  const filtered = onlinePlayers.filter((p) => stripAnsi(p.name).toLowerCase().includes(searchPlayer.toLowerCase()));
  ...
  const handleKickConfirm = async () => {
    ...
    try {
      ...
      toast.error(axiosError.response?.data?.message || `Failed to kick ${cleanedName}`);
    }
  };
  ...
  const handleOpConfirm = async () => {
    ...
    try {
      ...
      const axiosError = err as { response?: { data?: { message?: string } } };
      toast.error(axiosError.response?.data?.message || "Failed to update operator status");
    }
  };

  return (
    <Card className="flex min-h-[450px] animate-in flex-col border border-border/80 bg-card/65 p-5 duration-300 select-none fade-in lg:h-full">
        ...
        {!isOnline ? (
          ...
          (() => {
            if (filtered.length === 0) {
              return (
                <div className="py-12 text-center font-mono text-xs text-muted-foreground">{searchPlayer ? `No players found matching "${searchPlayer}"` : "No players are currently online."}</div>
              );
            }
            return (
                    ...
                    return (
                      <tr key={player.name} className="hover:bg-secondary/30">
                        <td className="flex items-center gap-2 py-2.5">
                          <div className="flex h-6 w-6 items-center justify-center rounded bg-primary/20 text-[10px] font-bold text-primary uppercase">{cleanedName.substring(0, 2)}</div>
                        ...
                        <td className="py-2.5 text-right">
                          <div className="flex items-center justify-end gap-1.5">
                            <Button
                              ...
                              className={`h-7 cursor-pointer border-border text-[10px] font-semibold ${
                                player.op ? "text-amber-500 hover:text-amber-600" : "text-muted-foreground hover:text-foreground"
                              }`}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to kick the player <span className="font-mono font-bold text-foreground">&quot;{kickDialog.player ? stripAnsi(kickDialog.player.name) : ""}&quot;</span>? They will
              be immediately disconnected from the game server.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={opDialog.isOpen} onOpenChange={(open) => !open && setOpDialog({ isOpen: false, player: null })}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to {opDialog.player?.op ? "remove" : "grant"} operator privileges for{" "}
              <span className="font-mono font-bold text-foreground">&quot;{opDialog.player ? stripAnsi(opDialog.player.name) : ""}&quot;</span>?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleOpConfirm}>Confirm</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </Card>
