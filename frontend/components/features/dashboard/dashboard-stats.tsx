...
export function DashboardStats() {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      />
      <MetricCard title="Total Players" value={`${totalPlayers} / ${maxPlayers}`} percentage={playerPercent} icon={Users} subtext="Live concurrent players count" />
      <MetricCard
      ...
      />
      <MetricCard title="Average CPU Load" value={`${avgCpuUsage.toFixed(1)}%`} percentage={cpuPercent} icon={Cpu} subtext={`Total allocated capacity: ${totalCpusLimit}%`} />
    </div>
  );
}
